...
public static partial class JSONTemplates {
    ...
    public static Keyframe ToKeyframe(JSONObject obj){
        // this was modified to compile, maybe everything else here that is a float needs modded too...
#if USEFLOAT
...
#else
        float dTime = System.Convert.ToSingle(obj.HasField("time") ? obj.GetField("time").n : 0);
        float dValue = System.Convert.ToSingle(obj.HasField("value") ? obj.GetField("value").n : 0);
        Keyframe k = new Keyframe(dTime, dValue);
        if (obj.HasField("inTangent")) k.inTangent = System.Convert.ToSingle(obj.GetField("inTangent").n);
        if (obj.HasField("outTangent")) k.outTangent = System.Convert.ToSingle(obj.GetField("outTangent").n);
        if (obj.HasField("tangentMode")) k.tangentMode = (int)obj.GetField("tangentMode").n;
#endif
		return k;
	}
